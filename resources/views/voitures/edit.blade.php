...
@section('content')
    <h1>Modifier la voiture : {{ $voiture->modele }}</h1>

    <form action="{{ route('voitures.update', $voiture) }}" method="POST">
        @csrf
        @method('PUT')
        ...
        <div class="mb-3">
            ...
            <select class="form-select @error('employe_id') is-invalid @enderror" id="employe_id" name="employe_id" required>
                ...
                @foreach($employes as $employe)
                    <option value="{{ $employe->id }}" {{ old('employe_id', $voiture->employe_id) == $employe->id ? 'selected' : '' }}>
        ...
        <div class="mb-3">
            ...
            <input type="number" class="form-control @error('nb_places') is-invalid @enderror" id="nb_places" name="nb_places" value="{{ old('nb_places', $voiture->nb_places) }}" min="1" required>
            @error('nb_places')
                <div class="invalid-feedback">{{ $message }}</div>
            @enderror
        </div>
